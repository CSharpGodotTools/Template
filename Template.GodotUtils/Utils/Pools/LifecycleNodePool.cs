...
/// <code>
/// // Create the pool
/// LifecycleNodePool&lt;Projectile&gt; pool = new(parentNode, () => projectilePackedScene.Instantiate());
/// 
...
public sealed class LifecycleNodePool<TNode> : IPoolContext<TNode>
    where TNode : CanvasItem, IPoolable<TNode>
{
    ...
    private static readonly Action<TNode> _onAcquire = static node => node.OnAcquire();
    private static readonly Action<TNode> _onRelease = static node => node.OnRelease();
