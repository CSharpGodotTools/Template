...
internal sealed class PacketGenerationOrchestrator
{
    ...
    public string? GenerateSource(Compilation compilation, INamedTypeSymbol symbol)
    {
        ...
        string? packetFrameworkNamespace = GetPacketFrameworkNamespace(symbol);
...
""";
    ...
    /// <summary>
    /// Resolves the namespace that contains the core packet framework types.
    /// This follows the packet inheritance chain and uses the namespace where
    /// ClientPacket/ServerPacket are declared.
    /// </summary>
    private static string? GetPacketFrameworkNamespace(INamedTypeSymbol symbol)
    {
    ...
    private static string? TryGetNamespaceFromUsings(ClassDeclarationSyntax classDeclaration)
    {
        ...
        string? netcodeNamespace = candidateUsings.FirstOrDefault(namespaceName => namespaceName.EndsWith(PacketGenConstants.NetcodeNamespaceSuffix));
        if (netcodeNamespace is not null)
            return netcodeNamespace;
